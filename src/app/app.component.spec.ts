...
describe(`AppComponent`, () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      declarations: [AppComponent, StudentListComponent, StudentInfoComponent, StudentCoursesComponent],
      imports: [FormsModule],
  ...
  describe('Functionality', () => {
    ...
    describe('Selecting a student', () => {
      beforeEach(async () => {
        ...
        const option: HTMLOptionElement = (Array.from(studentListSelect.children).find((child) => child.textContent.trim() === `${studentToSelect.firstName} ${studentToSelect.lastName}`) as HTMLOptionElement) || undefined;
      ...
      it(`should display the selected student's courses in alphabetical order by course name`, async () => {
        // arrange
        const courseIdsForSelectedStudent = studentCourses[studentToSelect.email];
        const courseNamesInAlphabeticalOrder = courses
          .filter((course) => courseIdsForSelectedStudent.includes(course.id))
        ...
        courseNamesInAlphabeticalOrder.forEach((courseName: string, index: number) => {
          const courseNameInListAtSameIndex = listItems[index].innerText.trim().split(' ')[0];
        ...
        courseIdsForSelectedStudent.forEach((courseId) => {
          const course = courses.find((c) => c.id === courseId);
          const expectedCourseText = `${course.name} - ${course.instructor} (${course.credits} credits)`;
          const listItemContainingCourse = listItems.find((listItem) => listItem.innerHTML.trim() === expectedCourseText);

          expect(listItemContainingCourse).toBeDefined();
          expect(listItemContainingCourse.innerHTML.trim()).toEqual(expectedCourseText);
        });
      });
